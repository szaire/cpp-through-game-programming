...
bool isMoveWrong(const vector<char>& visitedPos, ushort position);
bool isInsideList(const vector<char>& board, int position);
...
bool isMoveWrong(const vector<char>& list, ushort position)
{
    return (position < 0 || position > 8) || (count(list.begin(), list.end(), position));
}
...
void playerMove(vector<char>& board, vector<char>& visitedPos)
{
    ...
    while (isMoveWrong(visitedPos, position)) {
